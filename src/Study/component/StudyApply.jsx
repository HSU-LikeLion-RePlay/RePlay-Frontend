...
import { userState, studyInfoState } from '../../atoms';
import { Map, MapMarker } from "react-kakao-maps-sdk";
import locimg from '../images/Loc.svg';
import '../style/StudyApply.css';
...
export default function StudyApply() {
  ...
  const [studyInfos] = useRecoilState(studyInfoState); // 놀이터 정보 가져오기
  ...
  // 첫 번째 블록만 가져옴
  const firstBlock = studyInfos.length > 0 ? studyInfos[0] : null;
  ...
  return (
    <div>
      ...
      {showModal && <GuideLine show={showModal} onClose={handleCloseModal} isLoggedIn={isLoggedIn} />}
    </div>
  );
}
